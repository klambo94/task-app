...
def get_me(
    # current_user = Depends(get_current_user),  # wire in later
    user_id: str,  # temporary — pass user_id manually until auth is wired
...
def update_me(
    user_in: UserUpdate,
    user_id: str,  # temporary
    session: Session = Depends(get_db)
):
    user = user_repository.update(user_id, user_in, session)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return DataResponse(data=user)
...
def delete_me(
    ...
    success = user_repository.delete(user_id, session)
    if not success:
        raise HTTPException(status_code=404, detail="User not found")
