...
def create_sprint(
    ...
    if not access_repository.can_access_space(user_id, sprint_in.spaceId, session):
...
def get_active_sprint(space_id: str,
                      user_id: str,# Temp, once auth is hooked up use:  current_user = Depends(get_current_user)
                      session: Session = Depends(get_db)):
    if not access_repository.can_access_space(user_id, space_id, session):
        raise HTTPException(status_code=403, detail="Access denied")
...
def get_sprint(sprint_id: str,
               user_id: str, # Temp, once auth is hooked up use:  current_user = Depends(get_current_user)
               session: Session = Depends(get_db)):
    if not access_repository.can_access_sprint(user_id, sprint_id, session):
        raise HTTPException(status_code=403, detail="Access denied")
...
def get_sprint_statuses(space_id: str,
                        user_id: str, # Temp, once auth is hooked up use:  current_user = Depends(get_current_user)
                        session: Session = Depends(get_db)):
    if not access_repository.can_access_space(user_id, space_id, session):
        raise HTTPException(status_code=403, detail="Access denied")
...
def delete_sprint_status(status_id: str, session: Session = Depends(get_db)):
