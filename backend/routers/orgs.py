...
def get_orgs(
    ...
    orgs = organization_repository.get_by_user(session=session, user_id=current_user.id)
    if not orgs:
...
def create_org(
    ...
    org_in = OrganizationCreate(name=org_name, ownerId=current_user.id)
...
def update_org(
    ...
    if not access_repository.is_org_admin(current_user.id, org_id, session):
...
def delete_org(
    ...
    if not access_repository.is_org_owner(current_user.id, org_id, session):
...
def update_member_role(
    ...
    if member_in.role != OrgRole.OWNER and not access_repository.is_org_owner(current_user.id, org_id, session):
